...
/**
WHERE e.department_id=d.department_id 의 문구가 중요하다.
employees 테이블의 foreign key와 departments 테이블의 primary key를 매칭시켜 두 테이블의 레코드 값을 연결시킴.
**/
...
SELECT e.employee_id 사원번호, e.first_name 사원이름, d.department_name 부서명, e.hire_date 입사일
FROM employees e, departments d
WHERE e.department_id = d.department_id 
	AND e.department_id =50
	AND e.job_id IN (SELECT job_id FROM employees WHERE department_id=30);
-- e.job_id = (SELECT job_id FROM employees WHERE department_id=30); 는 안된다. = 대신에 IN을 넣어야 한다.
