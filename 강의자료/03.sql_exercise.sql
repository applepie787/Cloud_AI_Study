...
SELECT department_id 부서번호, first_name 이름, hire_date 입사일, CURDATE() 현재일,
       TO_DAYS(CURDATE())-TO_DAYS(hire_date) 근무일수,
       DATE_FORMAT(CURDATE(),'%Y')-DATE_FORMAT(hire_date,'%Y') 근무년수,
       DATE_FORMAT(hire_date,'%y') 입사년도,
       ROUND((TO_DAYS(CURDATE())-TO_DAYS(hire_date))/30, 0) 근무월수
FROM employees;   

-- CURDATE() 또는 SYSDATE() 또는 NOW().
...
-- mysql에서는 SUBSTR이 SUBSTRING으로. SUBSTRING(str, (시작점), (길이)) 이때, 시작점 값이 1일때, 첫글자.
