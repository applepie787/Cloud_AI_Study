...
class MemberMenuWindow(qtw.QDialog):
    ...
    def __init__(self): 
        ...
        self.dict_ = {}
    def setupUI(self):
        self.setGeometry(740, 280, 350, 260)

        groupBox = qtw.QGroupBox("{0}님 환영합니다. 메뉴를 선택해주세요".format(self.dict_["name"]), self)
        groupBox.move(10, 10)
        groupBox.resize(320, 260)
        ...
        self.button2 = qtw.QPushButton("-- 클럽 맴버쉽 탈퇴 --", self)
        self.button2.move(20, 110)
        self.button2.resize(300, 30)
        self.button2.clicked.connect(self.Button_2_Clicked)

        self.button3 = qtw.QPushButton("-- 커뮤니티 맴버 보기 --", self)
        self.button3.move(20, 160)
        self.button3.resize(300, 30)
        self.button3.clicked.connect(self.Button_3_Clicked)

        self.button4 = qtw.QPushButton("-- 메뉴창 닫기 --", self)
    ...
    def Button_1_Clicked(self):
        ...
        if(win_.showModal()):
            ...
            message_ = controller.register_controller(key_val, MemberShipEntity(key_val, clubID, self.dict_["email"], time_now_, "member"), "membership_")
            message_display(message_)

    def Button_2_Clicked(self):
        # 클럽 맴버쉽에 탈퇴하는 창 띄우기
        win_ = DeleteMembership()
        win_.initUI("membership_", str(self.dict_["email"]), "member_email")
        if(win_.showModal()):
...
class ClubMenuWindow(qtw.QDialog):
    ...
    def setupUI(self):
        self.setGeometry(740, 280, 350, 310)

        groupBox = qtw.QGroupBox("{0}클럽 관리자님. 메뉴를 선택해주세요".format(self.dict_["name"]), self)
        groupBox.move(10, 10)
        groupBox.resize(320, 260)
        ...
        self.button2 = qtw.QPushButton("-- 맴버쉽 등록 시키기 --", self)
        self.button2.move(20, 110)
        self.button2.resize(300, 30)
        self.button2.clicked.connect(self.Button_2_Clicked)

        self.button3 = qtw.QPushButton("-- 맴버쉽 탈퇴 시키기 --", self)
        self.button3.move(20, 160)
        self.button3.resize(300, 30)
        self.button3.clicked.connect(self.Button_3_Clicked)

        self.button3 = qtw.QPushButton("-- 커뮤니티 맴버 보기 --", self)
        self.button3.move(20, 210)
        self.button3.resize(300, 30)
        self.button3.clicked.connect(self.Button_4_Clicked)
    ...
    def Button_1_Clicked(self):
        ...
        win_ = ListPrintWindow()
        win_.initUI("membership_", self.dict_["clubID"], "clubID")
        win_.showModal()
        pass
    ...
    def Button_2_Clicked(self):
        ...
        if(win_.showModal()):
            ...
            message_ = controller.register_controller(key_val, MemberShipEntity(key_val, self.dict_["clubID"], member_email, time_now_, "member"), "membership_")
            message_display(message_)
        pass
