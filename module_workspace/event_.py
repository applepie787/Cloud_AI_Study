...
def register_entity(key, table_name):
    ...
    if(table_name == "member_"):
        register_win = GUI_windows.RegisterCommunityMember()
    elif(table_name == "club_"):
        register_win = GUI_windows.RegisterClub()
...
def register_membership(register_type, my_key):
    ...
    if(r and register_type == "member"):
        ...
        message_ = controller.register_controller(key_val, MemberShipEntity(key_val, clubID, my_key, time_now_, "member"), "membership_")

    elif(r and register_type == "club"):
        member_email = win_.line_1.text()
        message_ = controller.register_controller(key_val, MemberShipEntity(key_val, my_key, member_email, time_now_, "member"), "membership_")

    return message_


# 해당 메뉴(맴버/클럽)에서 entity값 수정.
